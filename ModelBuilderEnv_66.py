...
class ModelBuilderEnv_66:
    ...
    def _get_data(self):
        ...
        self.raw.index = self.raw.index.tz_localize('GMT')

        # drop the "complete column"  the data is all true

        self.raw.drop('complete', 1, inplace = True)
        ...
        # in subsequent iteration, remove weekend ticks for now.  These may behave dramatically different than weekday ticks. and may need a different rl model.
        self.raw = self.raw[self.raw.index.dayofweek <5]
        
        
        
        
        # now we will let self.data be the subset for either learning, validating or testing purposes self.raw will be available as original dataframe. 
    ...
    def _prepare_data(self):
        ...
        # self.raw = (self.raw - self.raw.min()) / (self.raw.max() - self.raw.min())
        # print(self.raw['r'])

           # populate history through lags to be added to features to for state

        
        self.features = self._add_lags()

        # don't want to normalize d column, it will be used as the "action", so 'd'  after normalization occurs.  Note, we may want to add a hold action later
        # here 1 = buy, 0 = sell.
        # the direction of the diffence in log return
        self.raw['d'] = np.where(self.raw['r'] > 0, 1, 0)
        # change direction from string to integer
        self.raw['d'] = self.raw['d'].astype(int)
        ...
        #             self.raw = self.raw.iloc[ :self.end-self.start]
        #             self.raw = self.raw.iloc[:self.end - self.start]
        # **********
    ...
    def _add_lags(self):
        ...
        for f in features:
            for lag in range(1, self.lags + 1):
                col = f'{f}_lag_{lag}'
                self.raw[col] = self.raw[f].shift(lag)
                cols.append(col)
        self.raw.dropna(inplace=True)
        self.features = cols
    ...
    def _get_state(self):
        ...
        # This is hilpisch's code which does not make much sense:
        # self.bar = self.lags
        ...
        return self.raw[self.features].iloc[self.bar].values
    ...
    def step(self, action):
        ...
        correct = (action == self.raw['d'].iloc[self.bar-self.lags])
        ...
        tradeReturn = self.raw['r'].iloc[self.bar] * self.leverage
        ...
        # print('self.min_accuracy: ', self.min_accuracy)
        # print('self.bar: ', self.bar)
        # print('length self.raw: ', len(self.raw))
        if self.bar >= (len(self.raw) - self.bar):
            self.done = True
            # print('Success:  self.bar >= len(self.raw); self.bar: ', self.bar)
        # elif correct == 1:
        #     done = False
        # elif (self.accuracy < self.min_accuracy and
        #       self.bar > self.lags + 15):
        elif (self.accuracy < self.min_accuracy) and (self.bar > 50):
            # print('Failed:  self.accuracy < self.min_accuracy; self.bar: ', self.bar)
            self.done = True
        # elif (self.performance < self.min_performance and
        #       self.bar > self.lags + 15):
        #     done = True
        elif (self.performance < self.min_performance) and (self.bar > 50):
            # print('Failed: self.performance < self.min_performance:  ', self.bar)
            self.done = True
        else:
            # print('accuracy and performance > minimum thresholds')
            self.done = False
